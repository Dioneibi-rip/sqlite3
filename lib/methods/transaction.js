...
module.exports.async = function transactionAsync(fn) {
	if (typeof fn !== 'function') throw new TypeError('Expected first argument to be a function');
	...
	const properties = {
		default: { value: wrapAsyncTransaction(apply, fn, db, controller.default) },
		deferred: { value: wrapAsyncTransaction(apply, fn, db, controller.deferred) },
		immediate: { value: wrapAsyncTransaction(apply, fn, db, controller.immediate) },
		exclusive: { value: wrapAsyncTransaction(apply, fn, db, controller.exclusive) },
		database: { value: this, enumerable: true },
	};
...
const wrapAsyncTransaction = (apply, fn, db, { begin, commit, rollback, savepoint, release, rollbackTo }) => async function sqliteAsyncTransaction() {
	let before, after, undo;
	if (db.inTransaction) {
		before = savepoint;
		after = release;
	...
	try {
		const result = await apply.call(fn, this, arguments);
		await after.runAsync();
		return result;
	} catch (ex) {
		if (db.inTransaction) {
			await undo.runAsync();
			if (undo !== rollback) await after.runAsync();
		}
		throw ex;
	}
};
